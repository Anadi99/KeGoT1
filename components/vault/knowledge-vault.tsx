...
export function KnowledgeVault({ entries, projectId }: KnowledgeVaultProps) {
      ...
      <Tabs defaultValue="all" onValueChange={(val) => setSelectedCategory(val as any)}>
        <TabsList className="grid grid-cols-5 w-full">
          ...
          <TabsTrigger value="decision">
            Decisions
            <Badge variant="secondary" className="ml-2">
              {categoryStats.decision}
            </Badge>
          </TabsTrigger>
          <TabsTrigger value="resource">
            Resources
            <Badge variant="secondary" className="ml-2">
              {categoryStats.resource}
            </Badge>
          </TabsTrigger>
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-yellow-600">{categoryStats.decision}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground mb-1">Resources</p>
              <p className="text-2xl font-bold text-blue-600">{categoryStats.resource}</p>
            </div>
            <div>
              ...
              <p className="text-sm font-semibold">
                {entries.length > 0
                  ? new Date(
                      Math.max(
                        ...entries.map((e) => new Date(e.updatedAt).getTime())
                      )
                    ).toLocaleDateString()
                  : 'N/A'}
              </p>
