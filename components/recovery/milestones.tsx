...
export function Milestones({ milestones }: MilestonesProps) {
  ...
  const totalProgress = Math.round(
    milestones.reduce((sum, m) => sum + m.percentComplete, 0) / Math.max(milestones.length, 1)
  )
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {completed} of {milestones.length} completed · {totalProgress}% overall progress
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            >
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 mt-1">
                  ...
                  {(milestone.status === 'planned' || milestone.status === 'in-progress') && (
